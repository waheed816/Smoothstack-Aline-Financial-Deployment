...
public class ApplicationService {
     ...
     * @throws ApplicationNotFoundException If application with the provided ID does
     *                                      not exist.
     */
    @PermitAll
    ...
    /**
     * Create new application with all brand-new applicants or all existing
     * applicants.
     * 
     * @param request          ApplyRequest dto with request information.
     * @param responseConsumer ApplicationResponseConsumer contains logic to run
     *                         after response is received
     *                         from the underwriting service and before the
     *                         ApplicationResponse object
     *                         is returned by the method.
     * @return ApplicationResponse containing the newly created applicants and the
     *         application status.
     * @apiNote This method will create all the CreateApplicant dto objects within
     *          the applicants' property first. If the applicants cannot be created
     *          for any reason, the process
     *          will stop and throw an error. However, if the ApplyRequest is
     *          flagged with <code>noApplicants</code>
     *          then it will use a list of ids of existing applicants, and it will
     *          create an application with those
     *          existing applicants instead. This will allow for a front end to
     *          create applicants first to verify
        ...
        if (request.getNoNewApplicants() == null || !request.getNoNewApplicants()) {
            ...
            Set<Long> applicantIds = Optional.ofNullable(request.getApplicantIds())
                    .orElseThrow(() -> new BadRequestException(
                            "Field 'noNewApplicants' was set to true but 'applicantIds' field does not exist in the request."));

            if (applicantIds.isEmpty())
                throw new BadRequestException(
                        "Field 'noNewApplicants' was set to true but no existing members/applicants were provided.");
     ...
     */
    @RoleIsManagement
    public PaginatedResponse<ApplicationResponse> getAllApplications(@NonNull Pageable pageable,
